...
public class HtmltoXMLConverter {
	enum states {andgt, attrtext, comment, endscript, endtag, entity, hexaentity, namedentity, numericentity, script, skipcdata, specialtag, tag, text, tillgt, tillinst, tillquote};
	...
	String s = null;
	public HtmltoXMLConverter(File aFile)
	{
		try{
			if(isRequiredfile(aFile))
			{
		...
		{
			e.printStackTrace();
			System.out.println("File not present at " + aFile.getAbsolutePath());
		} catch (IOException e) {
			e.printStackTrace();
			throw new RuntimeException("Exception occured while reading a file");			
		}
	}
	public HtmltoXMLConverter(String aString)
	{
		if(aString!=null)
			s = aString;
	}
	public HtmltoXMLConverter(URL url)
	{
		s = this.getStringBuffer(url).toString();
	}
	private StringBuffer getStringBuffer(InputStream inputStream) {
		...
		try {
			...
			throw new RuntimeException("Exception in reading inputStream", e);
		}
	}
	private StringBuffer getStringBuffer(URL url){
		try {
			return this.getStringBuffer(url.openStream());
		} catch (IOException e) {
			return new StringBuffer();
		}
	}
	public String Html2Xml()
	{		
	...
	{
		try{			
			if(Files.size(file.toPath())!=0 && file.canRead())
				return true;			
		}
		catch(IOException e)
		{
			PrintData.printErrorString("The file" + file + " is of zero KB");
			e.printStackTrace();
		}
		return false;
	}
